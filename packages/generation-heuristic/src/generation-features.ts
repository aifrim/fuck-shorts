 ...
 */
export function extractGenerationFeatures(
  raw: GenerationRawSignals,
): GenerationFeatures {
  ...
  const isWindows =
    includesAny(ua, ["Windows", "Win32", "Win64"]) ||
    platform.startsWith("Win");
  const isMacos =
    !isIos &&
    (includesAny(ua, ["Macintosh", "Mac OS X"]) ||
      platform.startsWith("Mac"));
  const isChromeos = includesAny(ua, ["CrOS", "ChromeOS"]);
  const isLinux =
    !isAndroid &&
    !isChromeos &&
    (ua.includes("Linux") || platform.startsWith("Linux"));
  ...
  const isFromTwitter = hostIncludes(raw.referrer, [
    "twitter.com",
    "x.com",
    "t.co",
  ]);
  const isFromYoutube = hostIncludes(raw.referrer, [
    "youtube.com",
    "youtu.be",
  ]);
  ...
  const isTiktokInApp = includesAny(ua, [
    "musical_ly",
    "TikTok",
    "BytedanceWebview",
  ]);
...
export function collectGenerationRawSignals(): GenerationRawSignals {
  ...
  const connection = (
    nav as Navigator & { connection?: NetworkInformationLike }
  ).connection;
