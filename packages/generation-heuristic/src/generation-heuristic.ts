...
/** Voices this package may return (never auto-picks generic). */
 ...
 */
export function scoreGenerationFeatures(
...
/**
 * Pick a voice from scores: max wins; tie / within MARGIN / below MIN_CONFIDENCE → genz.
 * Optional share-referrer bump when racing and not already in-app.
 */
export function pickVoiceFromFeatures(
  features: GenerationFeatures,
): HeuristicVoiceId {
  const scores = scoreGenerationFeatures(features);
  const ordered: HeuristicVoiceId[] = [
    "boomer",
    "millennial",
    "genz",
    "genalpha",
...
export function detectVoiceHeuristic(): HeuristicVoiceId {
  return pickVoiceFromFeatures(
    extractGenerationFeatures(collectGenerationRawSignals()),
  );
}
